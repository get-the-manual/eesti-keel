
# Числа


| #      | Mis?            | Mille?             | Mitmes? (какой/е?)     | Mitmendal? Kus? Millal? где? когда? в/на каком? | Millele? Kuhu? куда? на что? |
| ------ | --------------- | ------------------ | ---------------------- | ----------------------------------------------- | ---------------------------- |
| **1**  | üks             | ühe                | esimene                | esime**sel**                                    | esimesele                    |
| **2**  | kaks            | kahe               | teine                  | tei**sel**                                      | teisele                      |
| **3**  | kolm            | kolme              | kolma**s**             | kolma**ndal**                                   | kolma**ndale**               |
| **4**  | neli            | nelja              | nelja**s**             | nelja**ndal**                                   | nelja**ndale**               |
| **5**  | viis            | viie               | viie**s**              | viie**ndal**                                    | viie**ndale**                |
| **6**  | kuus            | kuue               | kuue**s**              | kuue**ndal**                                    | kuue**ndale**                |
| **7**  | seitse          | seitsme            | seitsme**s**           | seitsme**ndal**                                 | seitsme**ndale**             |
| **8**  | kaheksa         | kaheksa            | kaheksa**s**           | kaheksa**ndal**                                 | kaheksa**ndale**             |
| **9**  | üheksa          | üheksa             | üheksa**s**            | üheksa**ndal**                                  | üheksa**ndale**              |
| **10** | kümme           | kümne              | kümne**s**             | kümne**ndal**                                   | kümne**ndale**               |
| **11** | üksteist        | üheteistkümne      | üheteistkümne**s**     | üheteistkümne**ndal**                           | üheteistkümne**ndale**       |
| **12** | kaksteist       | kaheteistkümne     | kaheteistkümne**s**    | kaheteistkümne**ndal**                          | kaheteistkümne**ndale**      |
| **20** | kakskümmend     | kahekümne          | kahekümne**s**         | kahekümne**ndal**                               | kahekümne**ndale**           |
| **21** | kakskümmend üks | kahekümne ühe      | kahekümne esimene      | kahekümne esime**sel**                          | kahekümne esimesele          |
| **30** | kolmkümmend     | kolmekümne         | kolmekümne**s**        | kahekümne**ndal**                               | kahekümne**ndale**           |


## Правила применения

### **Mis? Mitu?** 
Просто называем число или считаем предметы (Nimetav).  

![](Mitu_tuba.png)


### **Mille?** (Omastav)

**Принадлежность или описание:** Когда число характеризует следующий за ним предмет.  
- Kolme korruse maja — Дом трех этажей (трехэтажный).  
- Kahe lapse isa — Отец двух детей.  

**С предлогами:** Большинство предлогов требуют именно этой формы.  
- Peale **kahe** tundi — После двух часов.  
- Enne **viie** päeva — До пяти дней.  

**когда к существительному добавляется -ga**  
- Tutvu ühe kinokavaga - Ознакомься с одной киноафишей  

Когда **есть действие** !! для числа **один**  
- Ma ostan **ühe** pileti. (Я `покупаю` один билет — здесь _ühe_, потому что есть действие покупки).  
- Ma annan sulle **ühe** toreda harjutuse - Я `дам` тебе одно замечательное упражнение

### **Mitmes?**

Какой по счету? Названия дат, этажей, мест в очереди.

**Даты:** число месяца — это всегда порядковое числительное.  
- Täna on **esimene** mai. — Сегодня первое мая.  

**Этажи (как название)**  
- See on **kolmas** korrus. — Это третий этаж.  

**Списки и места**  
- Ta sai võistlusel **teise** koha. — Он занял на соревновании второе место.

![](Mitmes_korrus.png)


### Mitmendal? Kus? Millal?

Ответ на вопросы:  
- На чём?  
- На каком по счету?  
- Какого числа? (место или время)  
- На каком этаже?  
- В каком году?  
- В какой день? 

Образуется по формуле: **Omastav + суффикс -l**.

- Ma elan **kolmandal** korrusel. (Я живу на третьем этаже).    

![](Mitmendal.png)  

### **Millele?** 

Направление или нахождение (на этаж, на время, к числу).  

Ответ на вопросы:  
- На какой по счету? 
- Куда? (направление)  
- На какой этаж?  
- К кому?  
- На какое время?  


Образуется по формуле: **Omastav + суффикс -le**.

**Направление движения**  
- Tõsta see **teisele** riiulile. (Положи это на вторую полку).  

**Время** (Ответ на вопрос «На какое время? / К какому часу?»)  
Используется, когда мы назначаем встречу или указываем временную границу.  
- Pane aeg **viiele**. (Запиши на пять [часов]).  
- Kohtume **kaheksale**. (Встретимся к восьми).  
    
**Передача кому-то** (Ответ на вопрос «Кому?»)  
Если число выступает в роли получателя (например, в математике или при распределении).  
- Lisa **kahele** kolm. (Прибавь к двум три).  
- Andsin **ühele** õuna и **teisele** pirni. (Дала одному яблоко, а второму грушу).  


## Составные числа

В составных числах (21, 35, 102) работает «правило очереди»:  
- **Только последнее слово** показывает, что число порядковое (Mitmes?).  
- **Все предыдущие слова** «замирают» в форме **Omastav**.  

| **Число** | **Mitmes?**       | **Mitmendal?**      |
| --------- | ------------------| --------------------|
| 20        | kahekümnes        | kahekümnendal       |
| 21        | kahekümne esimene | kahekümne esimesel  |
| 22        | kahekümne teine   | kahekümne teisel    |
| 30        | kolmekümnes       | kolmekümnendal      |
| 35        | kolmekümne viies  | kolmekümne viiendal |

## Прилагательные

Когда числительное становится частью другого слова. 
 
На вопрос `какая?` образуется по формуле: **Omastav + суффикс -line**.

`ühetoaline` korter. - однокомнатная квартира  
`Kahekorruseline` maja. - Двухэтажный дом.  
`viiekorruseline` maja. - пятиэтажный дом.  
`Mitmetoaline` korter see on? See on `kolmetoaline` korter. — Сколькикомнатная квартира это? Это - трехкомнатная квартира  
`Viieaastane` laps — Пятилетний (viie + aastane) ребенок.   


На вопрос `какую?` образуется по формуле: **Omastav + суффикс -list**  
Ma otsin `kahetoalist` korterit. - Я ищу двухкомнатную квартиру	


## Даты

Какое число? Какого числа?

![](Kuupaevad.jpg)

### **Mitmes?** (Какое число?)

`-s` и месяц в первой форме  
esimene jaanuar  
teine veebruar  
kolmas märts  
neljas aprill  
kümnes  
üheteistkümnes  
kaheteistkümnes  
viieteistkümnes  
kahekümnes  
kahekümne viies  

### **Mitmendal?** (какого числа?)

 `-dal` и месяц `-l`  
 esimesel jaanuaril  
 teisel veebruaril  
 kolmandal märtsil  
 üheksandal septembril  
 kümnendal oktoobril  
 kolmeteistkümnendal   
 kaheksateistkümnendal  
 kahekümne viiendal novembril  
 kolmekümnendal  
 kolmekümne esimesel
 


## Примеры

### Основа

Я говорю на `трех` языках	Ma räägin `kolme` keelt  
Дом `трех` этажей (трехэтажный)	`Kolme` korruse maja  
Ознакомься с `тремя` киноафишами Tutvu `kolme` kinokavaga  
После `трех` часов  Peale `kolme` tundi

### Порядок (какой? какое?)

Это `третий` этаж	See on `kolmas` korrus  
Сегодня `третье` мая.  Täna on `kolmas` mai  

### Порядок (какого? чьего?)

Обладатель **третьего места**.    `Kolmanda` koha omanik  
Кубок **третьего места**  `Kolmanda` koha karikas  

### Порядок (какого? на каком?)

Я живу `на третьем` этаже	Ma elan `kolmandal` korrusel  
Я родился `третьего` июня   Ma sündisin `kolmandal` juunil  

### Порядок  (куда? на какой?)

Идите `на третий` этаж Minge `kolmandale` korrusele  
Встретимся `третьего` числа  Kohtume `kolmandale`

### Количество (где? когда?)

Я был `на трех` концертах.   Ma olin `kolmel` kontserdil  
`В три` начинается собрание   `Kolmel` algab koosolek

### Количество (на сколько? кому?)

Встретимся `к трем`  Kohtume `kolmele`  
Я дал `трем` мальчикам конфеты   Ma andsin `kolmele` poisile kommi  

### Какой?

Maria vanematel on `kolmetoaline` korter. - У родителей Марии - трехкомнатная квартира  
Nende maja on `kolmekorruseline`. - Их дом - трехэтажный дом  
Marial on `ühetoaline` korter. - У Марии однокомнатная квартира  
See maja on `viiekorruseline`. - пятиэтажный дом

